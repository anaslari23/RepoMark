...
import { extractCapsule, buildAndEmbedCapsule, stripCapsule, toBase64Url, computeMarkerlessDigest } from './capsule.js';
import type { EmbeddedCapsulePayload } from './types.js';

test('extractCapsule: rejects capsules that are not the first comment', () => {
  const invalidCapsule = Buffer.from(
...
test('extractCapsule: rejects multiple capsules', () => {
  const multipleCapsules = Buffer.from(
    '/* @repomark-capsule-v1\na.b\n*/\n/* @repomark-capsule-v1\nc.d\n*/\nconsole.log("hello");',
    'utf8'
  );
