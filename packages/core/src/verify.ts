...
import { buildMerkleTree, computeLeafDigest, verifyMerkleProof } from './merkle.js';
import { defaultVerifier, type Signer } from './signing.js';
import {
  buildSourceOriginStatement,
  createDSSEEnvelope,
...
export function scanDirectory(
  dirPath: string,
  exclusions: string[] = ['.git', '.repomark', 'node_modules', 'dist', '.DS_Store']
): FileDigestEntry[] {
  const results: FileDigestEntry[] = [];
...
export function sealDirectory(
  ...
  const statementParams: BuildStatementParams = {
...
export function verifyEnvelope(
  envelopeInput: DSSEEnvelope | string,
  artifact: VerifyArtifactOptions,
  trustSnapshot?: TrustSnapshot,
  policy?: Partial<RepomarkPolicy>
): VerificationResult {
  ...
  let signerDecision: SignerDecision;
  ...
  if (revokedRecord || trustedRecord?.status === 'revoked') {
    ...
    let effStatus: KeyTrustStatus = 'REVOKED';
  ...
  if (pubKey) {
    signatureValid = defaultVerifier.verify(
  ...
  let actualFiles: FileDigestEntry[] = [];
  if (artifact.files) {
  ...
  const evidenceDigest = hashBytes(
    canonicalizeJSON({
      canonicalPayload,
      signer: signerDecision,
      merkleRootMatched,
 ...
 */
export function verifyPortableFile(
  fileBytes: Uint8Array | Buffer,
  trustSnapshot?: TrustSnapshot,
  _policy?: Partial<RepomarkPolicy>
): VerificationResult {
  const buf = Buffer.isBuffer(fileBytes) ? fileBytes : Buffer.from(fileBytes);

  let capsule: EmbeddedCapsule | null;
  try {
  ...
  if (pubKey) {
    ...
    try {
      signatureValid = defaultVerifier.verify(
        Buffer.from(b64Payload, 'utf8'),
        sigHex,
        pubKey,
        signerDecision.signatureAlgorithm
      );
    } catch {
      signatureValid = false;
    }
  ...
  const state: VerificationState = !signatureValid
    ? 'invalid'
