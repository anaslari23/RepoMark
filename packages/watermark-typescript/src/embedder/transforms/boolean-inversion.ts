import { Node, SyntaxKind, TypeChecker, PrefixUnaryExpression, BinaryExpression, IfStatement } from 'ts-morph';
 ...
 */
export const BooleanInversionRule: TransformRule = {
  ...
  isEligible(node: Node, checker: TypeChecker): boolean {
    if (Node.isIfStatement(node)) {
      ...
      if (Node.isBinaryExpression(expr) && expr.getOperatorToken().getKind() === SyntaxKind.EqualsEqualsEqualsToken) {
        ...
        if (leftType.isBoolean() && right.getText() === 'false') return true;
        if (rightType.isBoolean() && left.getText() === 'false') return true;
      }
    }
    return false;
  },
  ...
  applyVariant0(node: Node): Node {
    if (!Node.isIfStatement(node)) return node;
    const expr = node.getExpression();
  ...
  detectVariant(node: Node): 0 | 1 | -1 {
    if (!Node.isIfStatement(node)) return -1;
    const expr = node.getExpression();
