 ...
 */
export const ArrayIterationRule: TransformRule = {
  ...
  applyVariant0(node: Node): Node {
    if (Node.isForOfStatement(node)) return node;
  ...
  applyVariant1(node: Node): Node {
    if (Node.isExpressionStatement(node)) return node; // already forEach
  ...
  detectVariant(node: Node): 0 | 1 | -1 {
    if (Node.isForOfStatement(node)) return 0;
    if (Node.isExpressionStatement(node)) {
      const expr = node.getExpression();
      if (Node.isCallExpression(expr)) {
        const propAccess = expr.getExpression();
...
function containsForbiddenControlFlow(node: Node): boolean {
  let forbidden = false;
