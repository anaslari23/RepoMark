...
const AjvClass = _Ajv2020.default || _Ajv2020;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const addFormatsFn = _addFormats.default || _addFormats;
 ...
 */
export function buildSourceOriginStatement(params) {
    const claimKind = params.claimKind || 'verified-origin';
    const timestamp = params.timestamp || new Date().toISOString();
    const treeAlgorithm = params.treeAlgorithm || 'repomark-merkle-v1';
    let totalBytes = 0;
 ...
 */
export function parseAndValidateStatement(input) {
    let statementObj;
    ...
    if (!valid) {
        const errorMsg = validateStatementSchema.errors
            ? validateStatementSchema.errors.map((e) => `${e.instancePath} ${e.message}`).join(', ')
 ...
 */
export function createDSSEEnvelope(statement, signatureHexOrBase64, keyId) {
 ...
 */
export function unpackDSSEEnvelope(envelopeInput) {
    let envelope;
    if (typeof envelopeInput === 'string') {
        envelope = parseStrictJSON(envelopeInput);
    }
    else {
        envelope = parseStrictJSON(JSON.stringify(envelopeInput));
    }
