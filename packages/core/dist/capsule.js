...
export function extractCapsule(fileBytes) {
...
export function stripCapsule(fileBytes) {
    ...
    let skip = 0;
    if (after.length > 0 && after[0] === 0x0a)
        skip = 1; // \n
    else if (after.length > 1 && after[0] === 0x0d && after[1] === 0x0a)
        skip = 2; // \r\n
    return Buffer.concat([before, after.subarray(skip)]);
}
export function computeMarkerlessDigest(fileBytes) {
    const stripped = stripCapsule(fileBytes);
    return hashBytes(stripped, 'sha256');
}
export function buildEmbeddedCapsule(payload, signFn) {
...
export function buildAndEmbedCapsule(fileBytes, payload, signFn) {
