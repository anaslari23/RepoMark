 ...
 */
export function getEmptyMerkleRoot() {
 ...
 */
export function computeLeafDigest(canonicalPath, rawDigestHex) {
 ...
 */
export function computeParentDigest(leftHex, rightHex) {
...
export function buildMerkleTree(entries) {
    ...
    // 3. Build tree levels
    const levels = [leaves.map(l => l.leafDigest)];
