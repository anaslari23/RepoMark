...
import {
  ...
  type EmbeddedCapsulePayload,
  verifyPortableFile,
  computeMarkerlessDigest,
  buildAndEmbedCapsule
} from '@repomark/core';
import { MockKmsSigner } from './mock-signer.js';
import type { Signer } from '@repomark/core/src/signing.js';

const DEFAULT_POLICY: RepomarkPolicy = {
  version: 'v1',
...
function parseArgs(args: string[]): { command: string; options: Record<string, string | boolean>; positionals: string[] } {
...
// 1. INIT COMMAND
export function cmdInit(options: Record<string, string | boolean>): void {
...
// 2. SEAL COMMAND
export function cmdSeal(options: Record<string, string | boolean>): void {
  ...
  // Load Signer Key
  let signer: Signer;
  const signerType = options['signer'] as string;
  const customKeyPath = options['key'] as string;
  const privKeyPath = customKeyPath ? path.resolve(customKeyPath) : path.join(repomarkDir, 'private.key.pem');

  if (signerType === 'kms-mock') {
    const mockKeyId = (options['issuer-id'] as string) || 'mock-kms-key-1';
    signer = new MockKmsSigner(mockKeyId);
    
    // Auto-register in trust snapshot for convenience during CI simulation
    const trustPath = path.join(repomarkDir, 'trust.json');
    let trust: TrustSnapshot = { snapshotVersion: 'v1', updatedAt: new Date().toISOString(), trustedKeys: {} };
    if (fs.existsSync(trustPath)) {
      trust = parseStrictJSON<TrustSnapshot>(fs.readFileSync(trustPath, 'utf8'));
    }
    trust.trustedKeys[signer.keyId] = {
      keyId: signer.keyId,
      publicKey: signer.publicKey,
      algorithm: signer.algorithm,
      owner: (options['issuer-name'] as string) || 'Mock CI Workload Identity',
      status: 'active'
    };
    ...
    if (fs.existsSync(privKeyPath)) {
      ...
      trust.trustedKeys[keyPair.keyId] = {
        ...
        publicKey: keyPair.publicKey,
        algorithm: 'ed25519',
        owner: (options['issuer-name'] as string) || 'RepoMark Issuer',
        status: 'active'
      };
      fs.writeFileSync(trustPath, JSON.stringify(trust, null, 2) + '\\n');
    }
  ...
  const claimKind = (options['claim'] as ClaimKind) || policy.claimKind || 'verified-origin';
  const issuerId = (options['issuer-id'] as string) || `https://repomark.dev/issuers/${signer.keyId}`;
  const issuerName = (options['issuer-name'] as string) || 'Authorized Issuer';
  const organization = (options['org'] as string) || undefined;
...
// 3. VERIFY COMMAND
export function cmdVerify(options: Record<string, string | boolean>, positionals: string[]): VerificationResult {
  const targetPath = path.resolve((options['target'] as string) || positionals[0] || '.');

  let envelopeJson: string;
  let dirPath: string | undefined;
  let fileList: { path: string; rawDigest: string; size: number }[] | undefined;
  ...
  if (fs.existsSync(targetPath) && fs.statSync(targetPath).isFile() && targetPath.endsWith('.zip')) {
    ...
    const envEntry = extracted.find((e: ExtractedZipEntry) => e.path === '.repomark/envelope.json');
    if (!envEntry) {
    ...
    fileList = extracted
      .filter((e: ExtractedZipEntry) => !e.path.startsWith('.repomark/'))
      .map((e: ExtractedZipEntry) => ({
        path: e.path,
    ...
    if (targetPath.endsWith('envelope.json')) {
      ...
      const customTrust = options['trust'] as string;
  ...
  let policy: RepomarkPolicy = DEFAULT_POLICY;
  if (fs.existsSync(policyPath)) {
    policy = parseStrictJSON<RepomarkPolicy>(fs.readFileSync(policyPath, 'utf8'));
  }
  ...
  const result = verifyEnvelope(
    envelopeJson,
    fileList ? { files: fileList } : { directoryPath: dirPath },
    trustSnapshot,
    policy
  );
  ...
  if (options['json']) {
    ...
    const mismatches = result.subjectMatches.filter((m: SubjectMatch) => m.status !== 'matched');
...
// 4. INSPECT COMMAND
export function cmdInspect(options: Record<string, string | boolean>, positionals: string[]): void {
  const targetPath = path.resolve((options['target'] as string) || positionals[0] || '.repomark/statement.json');
...
export function cmdExport(options: Record<string, string | boolean>): string {
  const dir = path.resolve((options['dir'] as string) || '.');
  const outPath = path.resolve((options['out'] as string) || 'repomark-source.zip');
  ...
  // Scan directory
  const filesToPack: { path: string; data: Buffer }[] = [];
  ...
  function walk(currentDir: string, relativeDir: string) {
    ...
    for (const ent of entries) {
      ...
      if (ent.name === '.git' || normalized.startsWith('.git/')) continue;
...
// 6. MARK COMMAND
export function cmdMark(options: Record<string, string | boolean>, positionals: string[]): void {
  const targetPath = path.resolve((options['target'] as string) || positionals[0]);
  ...
  const policy = parseStrictJSON<RepomarkPolicy>(fs.readFileSync(policyPath, 'utf8'));
  const envelope = parseStrictJSON<DSSEEnvelope>(fs.readFileSync(envelopePath, 'utf8'));
  const statement = JSON.parse(Buffer.from(envelope.payload, 'base64').toString('utf8')) as InTotoStatement;
  ...
  const payload: EmbeddedCapsulePayload = {
  ...
  const newBytes = buildAndEmbedCapsule(fileBytes, payload, (buf: Buffer) => signer.sign(buf) as string);
...
// 7. ISSUE COMMAND
export async function cmdIssue(options: Record<string, string | boolean>, positionals: string[]): Promise<void> {
  const subcmd = positionals[0];
  const url = (options['url'] as string) || 'http://localhost:3001';
  ...
  if (subcmd === 'request') {
    ...
    const payloadTemplate: EmbeddedCapsulePayload = {
      issuerId: statement.predicate.issuer.id || '',
    ...
    if (!res.ok) throw new Error(data.error || 'Failed to request issuance');
    ...
    const reqId = options['req-id'] as string;
    const operator = options['operator'] as string;
    const file = options['file'] as string;
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Failed to approve issuance');
...
// 8. TRACE COMMAND
export async function cmdTrace(options: Record<string, string | boolean>, positionals: string[]): Promise<void> {
  const subcmd = positionals[0];
  const url = (options['url'] as string) || 'http://localhost:3001';

  if (subcmd === 'request') {
    const copyId = options['copy-id'] as string;
    const operator = options['operator'] as string;
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Failed to request trace');
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Failed to approve trace');
