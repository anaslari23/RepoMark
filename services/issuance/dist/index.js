...
import { Ed25519Signer, buildEmbeddedCapsule, deriveCopyId } from '@repomark/core';
...
const SIGNING_KEY_PEM = process.env.ISSUANCE_SIGNING_KEY || crypto.generateKeyPairSync('ed25519').privateKey.export({ type: 'pkcs8', format: 'pem' });
...
app.post('/issuance/approve/:requestId', (req, res) => {
    try {
        ...
        const payloadTemplate = JSON.parse(request.policy);
        ...
        const capsuleBlock = buildEmbeddedCapsule(payloadTemplate, (buf) => signer.sign(buf));
        // 4. Save approval and record in DB
        db.approveIssuanceRequest({ requestId, approverId, approvedAt: new Date().toISOString() }, {
            copyId,
            requestId,
